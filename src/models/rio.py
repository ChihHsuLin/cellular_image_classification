...
DEFAULT_IMAGES_BASE_PATH = DEFAULT_BASE_PATH  # os.path.join(DEFAULT_BASE_PATH, 'images')
...
def load_images_as_tensor(image_paths, dtype=np.uint8):
    ...
    for ix, img_path in enumerate(image_paths):
        data[:, :, ix] = imread(img_path, format='png')
...
def image_npy_path(dataset,
    ...
    Returns
    -------
    str the path of RGB image
    """
    return os.path.join(base_path, dataset, experiment, "Plate{}".format(plate), "{}_s{}.npy".format(address, site))
